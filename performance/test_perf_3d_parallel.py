    ...
    for n in range(5,6):
        ...
        for d in range(2, 8):
            d_d[d] = run_poisson_1( domain, solution, f,
                         ncells=[2**n,2**n,2**n], degree=[d,d,d],
...
def test_perf_poisson_2d_2():
    ...
    for d in range(2,8):
        d_f90[d] = run_poisson_1( domain, solution, f,
                         ncells=[2**4*p, 2**4*p,2**4*p], degree=[d, d, d],
                         backend=PSYDAC_BACKEND_GPYCCEL,nprocs=[p,p,p] )
...
def test_perf_poisson_2d_3():
    ...
    for d in range(2,8):
        d_f90[d] = run_poisson_2( domain, solution, f,
                         ncells=[2**8, 2**8, 2**8], degree=[d, d, d],
                         backend=PSYDAC_BACKEND_GPYCCEL )
